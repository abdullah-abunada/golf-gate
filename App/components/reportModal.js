...
class ReportModal extends Component {
  ...
  handleSend =()=>{
    ...
    if(why!='') 
    this.props.reportRequest(user_id, why,this.props.ad)
    else {
  ...
  renderContent = () => {
    ...
    return (
      <View style={{ flex: 1 }}>
      <View style={{ flex: 4 }}>
        <Text  style={{ ...Fonts.style.h5,alignSelf:'center',color:Colors.dark,marginBottom:20}}>{Strings.ar.report}</Text>
        <Item regular  style={styles.inputContainer} >
          <Input placeholder={"Strings.ar.why"} textBox style={{textAlign:'center'}}
            onChangeText={(value) => { this.props.handleInput('why', value) }}
            value={this.props.subject} />
        </Item>
        <Text style={{...Fonts.style.description,color:'red',margin:10,alignSelf:'center'}}>{this.props.error}</Text>
        <Text style={{...Fonts.style.description,color:'green',margin:10,alignSelf:'center'}}>{this.props.sentSuccess}</Text>
      </View>
      <View style={{ flex: 1, justifyContent: 'space-around', alignItems: 'center',flexDirection:'row'}}>
        {this.props.sentSuccess.length==0 &&<Button full dark onPress={this.handleSend}>
                                            <Text style={{ ...Fonts.style.h5,color:'white' }}>{Strings.ar.send}</Text>
                                   </Button>}
            {this.props.sentSuccess.length > 0 &&<Text style={{ ...Fonts.style.h5,color:Colors.green }}>{Strings.ar.success.sentSuccessfully}</Text>}
  ...
  render() {
    return (
        ...
        backdrop={false}
        coverScreen={true}>
      <View style={styles.container} enableOnAndroid>
        {this.renderContent()}
      </View>
      </ModalBox>
...
const mapStateToProps = ({ contact,auth }) => {
  const { why, fetching,error } = contact
  return {
